...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    if (H == 33){
        ...
        size_t constexpr shared_mem_size = (TILE_WIDTH_K1 + K - 1) * (TILE_WIDTH_K1 + K) * sizeof(float);
        ...
        dim3 gridDimNew(B, M, Y);
        // size is x_TILE_WIDTH * x_TILE_WIDTH_PADDED * sizeof(float)
        size_t constexpr shared_mem_size = (TILE_WIDTH_K2 + K - 1) * (TILE_WIDTH_K2 + K) * sizeof(float);
